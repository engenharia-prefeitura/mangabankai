...
async function runPtScrape(signal) {
  ...
  try {
    ...
    // Fase 5: Mugiwaras (Português)
    const mugiStep = s.subSteps && s.subSteps.find(step => step.id === 'mugiwaras');
    if (!mugiStep || mugiStep.status !== 'done') {
      log('pt', '🚀 Iniciando Fase 5: Varredura Mugiwaras...');
      await runExternalMadara('mugiwaras', 'pt', signal, 'mugiwaras');
      if (signal.aborted) throw new Error('Aborted');
    } else {
      log('pt', '⏭️ Fase 5 (Mugiwaras) já concluída. Pulando...');
    }

    flushSave(); s.status = 'done'; s.current = [];
    log('pt', `🎉 PT concluído com sucesso nas cinco fontes!`, 'success');
  } catch(e) {
    flushSave();
