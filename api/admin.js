...
async function pushSend(req, res) {
  ...
  webpush.setVapidDetails(
    process.env.VAPID_SUBJECT || 'mailto:noreply@mangabankai.com',
...
// ── takedowns — lista solicitações de desindexação ─────────────────────
async function takedowns(req, res) {
  if (req.method !== 'GET') return res.status(405).json({ error: 'Método não permitido' });
  if (!await isAdmin(req)) return res.status(403).json({ error: 'Forbidden' });
  const sql = await ensureConnection();
  const r = await sql`
    SELECT t.id, t.manga_id, t.full_name, t.contact_email, t.relationship,
           t.proof_url, t.details, t.status, t.admin_note, t.created_at, t.resolved_at,
           u.username
    FROM takedown_requests t LEFT JOIN users u ON u.id = t.user_id
    ORDER BY (t.status = 'pending') DESC, t.created_at DESC
    LIMIT 200
  `;
  const idx = getMangaIndex();
  const titleOf = id => { const m = idx.find(x => x.id === id); return m ? m.title : id; };
  return res.status(200).json({
    requests: (r.rows || []).map(t => ({ ...t, manga_title: titleOf(t.manga_id) }))
  });
}

// ── takedown-resolve — aprova (desindexa via hidden_manga) ou recusa ───
async function takedownResolve(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Método não permitido' });
  if (!await isAdmin(req)) return res.status(403).json({ error: 'Forbidden' });
  const { id, approve, note } = req.body || {};
  const reqId = parseInt(id, 10);
  if (isNaN(reqId)) return res.status(400).json({ error: 'id obrigatório' });
  const sql = await ensureConnection();
  const r = await sql`SELECT manga_id, status FROM takedown_requests WHERE id = ${reqId} LIMIT 1`;
  if (!r.rows || !r.rows[0]) return res.status(404).json({ error: 'Solicitação não encontrada' });
  const status = approve ? 'approved' : 'rejected';
  await sql`
    UPDATE takedown_requests
    SET status = ${status}, admin_note = ${String(note || '').trim() || null}, resolved_at = CURRENT_TIMESTAMP
    WHERE id = ${reqId}
  `;
  if (approve) {
    await sql`INSERT INTO hidden_manga (manga_id) VALUES (${r.rows[0].manga_id}) ON CONFLICT (manga_id) DO NOTHING`;
  }
