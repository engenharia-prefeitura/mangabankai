...
const ALL_SOURCES = [
  ...
  { name: 'mugiwaras',     domain: 'mugiwarasoficial.com', cpt: 'manga',  lang: 'pt', adult: false }
];
...
function checkMugiwarasScraperImageHead(url) {
...
async function parseMugiwarasPages(html) {
  try {
    ...
    const extMatch = page1Url.match(/\.([a-z0-9]+)(?:[\?#]|$)/i);
    ...
    for (let i = 1; i <= 100; i++) {
      ...
      pageChecks.push(checkMugiwarasScraperImageHead(url).then(exists => exists ? url : null));
    }
...
async function produceManga(src, slug) {
  ...
  for (const ch of chapters) {
    ...
    if (exist && ((exist.pages && exist.pages.length > 0) || exist.baseFolder)) {
  ...
  if (chaptersToScrape.length > 0) {
    ...
    const scrapeWorker = async () => {
      while (nextChIdx < chaptersToScrape.length) {
        ...
        let baseFolder = '';
        let ext = '';
        if (src.name === 'mugiwaras') {
          const match = chHtml.match(/redenovax\.com\/jump\/[^?]+\?a=([^&"]+)/);
          if (match) {
            const page1Url = decodeURIComponent(match[1]);
            const lastSlash = page1Url.lastIndexOf('/');
            baseFolder = page1Url.substring(0, lastSlash + 1);
            const extMatch = page1Url.match(/\.([a-z0-9]+)(?:[\?#]|$)/i);
            ext = extMatch ? extMatch[1] : 'webp';
          }
        } else {
          pages = parsePages(chHtml);
        }
        if (!pages.length && !baseFolder) continue;
        chObj[src.lang].push({
          id: `${id}-ch-${ch.number}`, number: ch.number, title: ch.title,
          date: new Date().toISOString().slice(0, 10), pages, src: src.name, chapterUrl: ch.url,
          baseFolder, ext
